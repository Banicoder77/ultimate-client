...
import ultimatedevelopment.ultimateclient.gui.DefaultSettingsWidgetFactory;
...
import ultimatedevelopment.ultimateclient.gui.themes.ultimate.widgets.*;
import ultimatedevelopment.ultimateclient.gui.themes.ultimate.widgets.input.WUltimateDropdown;
import ultimatedevelopment.ultimateclient.gui.themes.ultimate.widgets.input.WUltimateSlider;
import ultimatedevelopment.ultimateclient.gui.themes.ultimate.widgets.input.WUltimateTextBox;
import ultimatedevelopment.ultimateclient.gui.themes.ultimate.widgets.pressable.*;
...
import ultimatedevelopment.ultimateclient.gui.widgets.*;
...
import ultimatedevelopment.ultimateclient.gui.widgets.input.WSlider;
import ultimatedevelopment.ultimateclient.gui.widgets.input.WTextBox;
import ultimatedevelopment.ultimateclient.gui.widgets.pressable.*;
...
public class UltimateGuiTheme extends GuiTheme {
    ...
    public WWindow window(WWidget icon, String title) {
        return w(new WUltimateWindow(icon, title));
    }

    @Override
    public WLabel label(String text, boolean title, double maxWidth) {
        if (maxWidth == 0 && !text.contains("\n")) return w(new WUltimateLabel(text, title));
        return w(new WUltimateMultiLabel(text, title, maxWidth));
    }

    @Override
    public WHorizontalSeparator horizontalSeparator(String text) {
        return w(new WUltimateHorizontalSeparator(text));
    }

    @Override
    public WVerticalSeparator verticalSeparator() {
        return w(new WUltimateVerticalSeparator());
    }

    @Override
    protected WButton button(String text, GuiTexture texture) {
        return w(new WUltimateButton(text, texture));
    }

    @Override
    protected WConfirmedButton confirmedButton(String text, String confirmText, GuiTexture texture) {
        return w(new WUltimateConfirmedButton(text, confirmText, texture));
    }

    @Override
    public WMinus minus() {
        return w(new WUltimateMinus());
    }

    @Override
    public WConfirmedMinus confirmedMinus() {
        return w(new WUltimateConfirmedMinus());
    }

    @Override
    public WPlus plus() {
        return w(new WUltimatePlus());
    }

    @Override
    public WCheckbox checkbox(boolean checked) {
        return w(new WUltimateCheckbox(checked));
    }

    @Override
    public WSlider slider(double value, double min, double max) {
        return w(new WUltimateSlider(value, min, max));
    }

    @Override
    public WTextBox textBox(String text, String placeholder, CharFilter filter, Class<? extends WTextBox.Renderer> renderer) {
        return w(new WUltimateTextBox(text, placeholder, filter, renderer));
    }

    @Override
    public <T> WDropdown<T> dropdown(T[] values, T value) {
        return w(new WUltimateDropdown<>(values, value));
    }

    @Override
    public WTriangle triangle() {
        return w(new WUltimateTriangle());
    }

    @Override
    public WTooltip tooltip(String text) {
        return w(new WUltimateTooltip(text));
    }

    @Override
    public WView view() {
        return w(new WUltimateView());
    }

    @Override
    public WSection section(String title, boolean expanded, WWidget headerWidget) {
        return w(new WUltimateSection(title, expanded, headerWidget));
    }

    @Override
    public WAccount account(WidgetScreen screen, Account<?> account) {
        return w(new WUltimateAccount(screen, account));
    }

    @Override
    public WWidget module(Module module, String title) {
        return w(new WUltimateModule(module, title));
    }

    @Override
    public WQuad quad(Color color) {
        return w(new WUltimateQuad(color));
    }

    @Override
    public WTopBar topBar() {
        return w(new WUltimateTopBar());
    }

    @Override
    public WFavorite favorite(boolean checked) {
        return w(new WUltimateFavorite(checked));
    }
