...
import ultimatedevelopment.ultimateclient.utils.render.UltimateToast;
...
public class TitleScreenCredits {
    ...
    private static void init() {
        // Add addons
        for (UltimateAddon addon : AddonManager.ADDONS) add(addon);

        // Sort by width (Meteor always first)
        credits.sort(Comparator.comparingInt(value -> value.addon == UltimateClient.ADDON ? Integer.MIN_VALUE : -mc.textRenderer.getWidth(value.text)));
        ...
        UltimateExecutor.execute(() -> {
            for (Credit credit : credits) {
                ...
                request.exceptionHandler(e -> UltimateClient.LOG.error("Could not fetch repository information for addon '{}'.", credit.addon.name, e));
                ...
                switch (res.statusCode()) {
                    case Http.UNAUTHORIZED -> {
                        ...
                        UltimateToast toast = new UltimateToast.Builder("GitHub: Unauthorized").icon(Items.BARRIER).text(message).build();
                        mc.getToastManager().add(toast);
                        UltimateClient.LOG.warn(message);
                        if (System.getenv("meteor.github.authorization") == null) {
                            UltimateClient.LOG.info("Consider setting an authorization " +
                                "token with the 'meteor.github.authorization' environment variable.");
                            UltimateClient.LOG.info("See: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens");
                        }
                    }
                    case Http.FORBIDDEN -> UltimateClient.LOG.warn("Could not fetch updates for addon '{}': Rate-limited by GitHub.", credit.addon.name);
                    case Http.NOT_FOUND -> UltimateClient.LOG.warn("Could not fetch updates for addon '{}': GitHub repository '{}' not found.", credit.addon.name, repo.getOwnerName());
                    case Http.SUCCESS -> {
    ...
    private static void add(UltimateAddon addon) {
    ...
    private static class Credit {
        public final UltimateAddon addon;
        public final MutableText text = Text.empty();

        public Credit(UltimateAddon addon) {
            this.addon = addon;
        }
    }
