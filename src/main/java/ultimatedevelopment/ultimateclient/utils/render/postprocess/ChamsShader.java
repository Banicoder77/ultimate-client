...
import ultimatedevelopment.ultimateclient.systems.modules.render.Chams;
...
public class ChamsShader extends EntityShader {
    ...
    public ChamsShader() {
        super(UltimateRenderPipelines.POST_IMAGE);
        UltimateClient.EVENT_BUS.subscribe(ChamsShader.class);
    }
    ...
    public static void load() {
        try {
            ...
            for (String fileFormat : FILE_FORMATS) {
                Optional<Resource> optional = mc.getResourceManager().getResource(UltimateClient.identifier("textures/chams." + fileFormat));
            ...
            UltimateClient.LOG.error("Error loading the chams shader", e);
        }
    }
    ...
    private static final DynamicUniformStorage<UniformData> UNIFORM_STORAGE = new DynamicUniformStorage<>("Ultimate - Image UBO", UNIFORM_SIZE, 16);
