        ...
        if (indexCount > 0) {
...

            {
                ...
                RenderPass pass = (depthAttachment != null && pipeline.wantsDepthTexture()) ?
                    RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Ultimate MeshRenderer", colorAttachment, clearColor, depthAttachment, OptionalDouble.empty()) :
                    RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Ultimate MeshRenderer", colorAttachment, clearColor);
