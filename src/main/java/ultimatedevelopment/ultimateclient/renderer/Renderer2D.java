...
import ultimatedevelopment.ultimateclient.gui.renderer.packer.TextureRegion;
...
public class Renderer2D {
    ...
    public Renderer2D(boolean textured) {
        ...
        triangles = new MeshBuilder(textured ? UltimateRenderPipelines.UI_TEXTURED : UltimateRenderPipelines.UI_COLORED);
        lines = new MeshBuilder(UltimateRenderPipelines.UI_COLORED_LINES);
    }
    ...
    public void render(String samplerName, GpuTextureView samplerView, GpuSampler sampler) {
        ...
        MeshRenderer.begin()
            ...
            .pipeline(UltimateRenderPipelines.UI_COLORED_LINES)
            .mesh(lines)
            .end();

        MeshRenderer.begin()
            .attachments(MinecraftClient.getInstance().getFramebuffer())
            .pipeline(textured ? UltimateRenderPipelines.UI_TEXTURED : UltimateRenderPipelines.UI_COLORED)
            .mesh(triangles)
