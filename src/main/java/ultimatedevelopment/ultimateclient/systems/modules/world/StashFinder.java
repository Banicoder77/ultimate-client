...
import ultimatedevelopment.ultimateclient.utils.misc.text.RunnableClickEvent;
...
public class StashFinder extends Module {
    ...
    public StashFinder() {
        super(Categories.World, "stash-finder", "Searches loaded chunks for storage blocks. Saves to <your minecraft folder>/ultimate-client");
    }
        ...
        if (chunk.getTotal() >= minimumStorageCount.get()) {
            ...
            if (sendNotifications.get() && (!chunk.equals(prevChunk) || !chunk.countsEqual(prevChunk))) {
                switch (notificationMode.get()) {
                    ...
                    case Toast -> {
                        UltimateToast toast = new UltimateToast.Builder(title).icon(Items.CHEST).text("Found Stash!").build();
    ...
    private void saveCsv() {
        try {
            ...
            UltimateClient.LOG.error("Error while writing the stash list to csv", e);
        }
    }
    ...
    private void saveJson() {
        try {
            ...
            UltimateClient.LOG.error("Error while writing the stash list to json", e);
        }
    }

    private File getJsonFile() {
        return new File(new File(new File(UltimateClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.json");
    }

    private File getCsvFile() {
        return new File(new File(new File(UltimateClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.csv");
    }
