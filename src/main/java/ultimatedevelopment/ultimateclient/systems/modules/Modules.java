...
import ultimatedevelopment.ultimateclient.systems.modules.render.*;
import ultimatedevelopment.ultimateclient.systems.modules.render.blockesp.BlockESP;
import ultimatedevelopment.ultimateclient.systems.modules.render.marker.Marker;
import ultimatedevelopment.ultimateclient.systems.modules.world.*;
...
import ultimatedevelopment.ultimateclient.utils.misc.ValueComparableMap;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                UltimateClient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    /***
     * @see ultimatedevelopment.ultimateclient.commands.commands.BindCommand
    ...
    private boolean onBinding(boolean isKey, int value, int modifiers) {
        ...
        UltimateClient.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
        moduleToBind = null;
        ...
        synchronized (active) {
            for (Module module : getAll()) {
                if (module.isActive() && !module.runInMainMenu) {
                    UltimateClient.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : getAll()) {
                if (module.isActive() && !module.runInMainMenu) {
                    UltimateClient.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
