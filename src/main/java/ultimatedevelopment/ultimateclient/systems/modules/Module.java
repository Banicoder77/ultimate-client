...
public abstract class Module implements ISerializable<Module>, Comparable<Module> {
    ...
    public Module(Category category, String name, String description, String... aliases) {
        if (name.contains(" ")) UltimateClient.LOG.warn("Module '{}' contains invalid characters in its name making it incompatible with Ultimate Client commands.", name);
        ...
        for (UltimateAddon addon : AddonManager.ADDONS) {
    ...
    public void toggle() {
        if (!active) {
            ...
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) UltimateClient.EVENT_BUS.subscribe(this);
                onActivate();
            }
        }
        else {
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) UltimateClient.EVENT_BUS.unsubscribe(this);
                onDeactivate();
            }
