...
import ultimatedevelopment.ultimateclient.renderer.FixedUniformStorage;
...
import ultimatedevelopment.ultimateclient.settings.IntSetting;
...
public class Blur extends Module {
    ...
    private final Setting<Boolean> meteor = sgScreens.add(new BoolSetting.Builder()
        .name("meteor")
        .description("Applies blur to Ultimate screens.")
        .defaultValue(true)
        .build());
    ...
    public Blur() {
        ...
        UltimateClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] != null) {
                    fbos[i].close();
        ...
        UltimateClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
    ...
    private void onRenderAfterWorld() {
        ...
        // Initial downsample
        renderToFbo(fbos[0], mc.getFramebuffer().getColorAttachmentView(), UltimateRenderPipelines.BLUR_DOWN, ubos[0]);

        // Downsample
        for (int i = 0; i < iterations; i++) {
            renderToFbo(fbos[i + 1], fbos[i], UltimateRenderPipelines.BLUR_DOWN, ubos[i + 1]);
        }

        // Upsample
        for (int i = iterations; i >= 1; i--) {
            renderToFbo(fbos[i - 1], fbos[i], UltimateRenderPipelines.BLUR_UP, ubos[i - 1]);
        }

        // Render output
        MeshRenderer.begin()
            .attachments(mc.getFramebuffer())
            .pipeline(UltimateRenderPipelines.BLUR_PASSTHROUGH)
            .fullscreen()
    ...
    private static final FixedUniformStorage<BlurUniformData> UNIFORM_STORAGE = new FixedUniformStorage<>("Ultimate - Blur UBO", UNIFORM_SIZE, 6);
