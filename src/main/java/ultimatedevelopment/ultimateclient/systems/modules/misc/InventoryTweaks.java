...
import ultimatedevelopment.ultimateclient.mixin.HandledScreenAccessor;
...
public class InventoryTweaks extends Module {
        ...
        for (int i = start; i < end; i++) {
            ...
            if (sleep > 0) {
                try {
                    ...
                    UltimateClient.LOG.error("Error when sleeping the slot mover", e);
                }
            }
    ...
    public void steal(ScreenHandler handler) {
        UltimateExecutor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(ScreenHandler handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        UltimateExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
