...
import ultimatedevelopment.ultimateclient.gui.utils.StarscriptTextBoxRenderer;
...
public class DiscordPresence extends Module {
    ...
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder()
        ...
        .defaultValue("Ultimate on Crack!", "{round(server.tps, 1)} TPS", "Playing on {server.difficulty} difficulty.", "{server.player_count} Players online")
    ...
    public void onActivate() {
        ...
        String largeText = "%s %s".formatted(UltimateClient.NAME, UltimateClient.VERSION);
        if (!UltimateClient.BUILD_NUMBER.isEmpty()) largeText += " Build: " + UltimateClient.BUILD_NUMBER;
        rpc.setLargeImage("ultimate_client", largeText);
    ...
    private void recompile(List<String> messages, List<Script> scripts) {
        ...
        for (String message : messages) {
            Script script = UltimateStarscript.compile(message);
        ...
        if (Utils.canUpdate()) {
            ...
            if (line1Ticks >= line1UpdateDelay.get() || forceUpdate) {
                if (!line1Scripts.isEmpty()) {
                    ...
                    String message = UltimateStarscript.run(line1Scripts.get(i));
            ...
            if (line2Ticks >= line2UpdateDelay.get() || forceUpdate) {
                if (!line2Scripts.isEmpty()) {
                    ...
                    String message = UltimateStarscript.run(line2Scripts.get(i));
            ...
            if (!lastWasInMainMenu) {
                rpc.setDetails(UltimateClient.NAME + " " + (UltimateClient.BUILD_NUMBER.isEmpty() ? UltimateClient.VERSION : UltimateClient.VERSION + " " + UltimateClient.BUILD_NUMBER));
