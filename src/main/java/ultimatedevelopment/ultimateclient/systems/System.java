...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(UltimateClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(UltimateClient.MOD_ID, file.getName());
            ...
            UltimateClient.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    UltimateClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    UltimateClient.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            UltimateClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
