...
package ultimatedevelopment.ultimateclient.systems.friends;
...
import ultimatedevelopment.ultimateclient.utils.network.FailedHttpResponse;
    ...
    public void updateInfo() {
        ...
        if (id != null) {
            res = Http.get("https://sessionserver.mojang.com/session/minecraft/profile/" + UndashedUuid.toString(id))
                .exceptionHandler(e -> UltimateClient.LOG.error("Error while trying to connect session server for friend '{}'", name))
        ...
        if (res == null || res.statusCode() != 200) {
            res = Http.get("https://api.mojang.com/users/profiles/minecraft/" + name)
                .exceptionHandler(e -> UltimateClient.LOG.error("Error while trying to update info for friend '{}'", name))
                .sendJsonResponse(APIResponse.class);
        }
